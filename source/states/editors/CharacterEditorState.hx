...
class CharacterEditorState extends MusicBeatState
{
	...
	var fpsPrefsBoxChecked:FlxUICheckBox;
	...
	function addAnimationsUI() {
		...
		fpsPrefsBoxChecked = new FlxUICheckBox(animationInputText.x + 270, animationInputText.y, null, null, "FPS Prefs", 80);
		fpsPrefsBoxChecked.checked = char.fpsPrefs;
		fpsPrefsBoxChecked.callback = function() {
			if(!fpsPrefsBoxChecked.checked) {
				char.fpsPrefs = true;
			}
		};
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			var newAnim:AnimArray = {
				...
				fpsPrefs: fpsPrefsBoxChecked.checked,
				indices: indices,
				offsets: lastOffsets
			};
		...
		tab_group.add(new FlxText(fpsPrefsBoxChecked.x, fpsPrefsBoxChecked.y - 18, 0, 'FpsPrefs'));
		...
		tab_group.add(fpsPrefsBoxChecked);
